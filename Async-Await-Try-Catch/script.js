...
function fonctionAuClic() {
    ...
    async function resaComplete() {
        try {
          const volResult = await resaVol();
          const hotelResult = await resaHotel();
          const voitureResult = await resaVoiture();
          console.log(volResult, hotelResult, voitureResult);
          finalMessage.innerHTML = `${volResult}, ${hotelResult}, ${voitureResult}`
          finalMessage.style.color = "green";
        } catch (erreur) {
            finalMessage.innerText = erreur;
...
//mettre les async,await et try,catch à la place
